...
async def metadata(file):
    ...
    try:
        ...
        data["performer"] = (
            info[0].get("Performer") or mrunal.get("artist") or beastx_bot.me.first_name
        )
        if len(info) >= 2:
...
async def get_chatbot_reply(message):
    chatbot_base = "https://kukiapi.xyz/api/apikey=BeastXUSERBOT/BeastX/{}/message={}"
    req_link = chatbot_base.format(
        beastx_bot.me.first_name or "BeastX user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    token = mrunal.get("_TELEGRAPH_TOKEN")
    ...
    short_name = gd_name if len(gd_name) < 32 else "BeastX"
    profile_url = (
    ...
    try:
        ...
        LOGS.exception(er)
        return
    mrunal.set("_TELEGRAPH_TOKEN", TelegraphClient.get_access_token())
    TELEGRAPH.append(TelegraphClient)
    return TelegraphClient
