...
async def allcmds(event, telegraph):
    ...
    for z in LIST.keys():
        ...
        for zz in LIST[z]:
            txt += HNDLR + zz + "\n"
        txt += "\n\n"
    t = telegraph.create_page(title="BeastX All Cmds", content=[txt])
    await eor(event, f"All BeastX Cmds : [Click Here]({t['url']})", link_preview=False)


async def ReTrieveFile(input_file_name):
    RMBG_API = mrunal.get("RMBG_API")
...
async def create_instagram_client(event):
    ...
    username = mrunal.get("INSTA_USERNAME")
    password = mrunal.get("INSTA_PASSWORD")
    if not (username and password):
        return
    settings = eval(mrunal.get("INSTA_SET")) if mrunal.get("INSTA_SET") else {}
    ...
    try:
        ...
        return False
    mrunal.set("INSTA_SET", str(cl.get_settings()))
