...
async def safeinstall(event):
    ...
    if plug in HELP:
        ...
        for i in HELP[plug]:
            output += i
        output += "\n© @TheBeastX"
        await eod(ok, f"✓ `BeastX - Installed`: `{plug}` ✓\n\n{output}")
...
def gen_chlog(repo, diff):
    ...
    ch_log = tldr_log = ""
    ch = f"<b>BeastX {BeastX_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
    ch_tl = f"BeastX {BeastX_version} updates for {ac_br}:"
    d_form = "%d/%m/%y || %H:%M"
...
async def restart(ult):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        os.execl(sys.executable, sys.executable, "-m", "BeastX-Py")


async def shutdown(ult, dynotype="BeastX"):
    from .. import LOGS
...
async def heroku_logs(event):
    ...
    ok = app.get_log()
    with open("BeastX-heroku.log", "w") as log:
        log.write(ok)
    await event.client.send_file(
        event.chat_id,
        file="BeastX-heroku.log",
        thumb="resources/extras/BeastX.jpg",
        caption="**BeastX Heroku Logs.**",
    )

    os.remove("BeastX-heroku.log")
    await xx.delete()
