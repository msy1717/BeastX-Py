...
from .. import DUAL_HNDLR, HNDLR, LOGS, SUDO_HNDLR, asst, mrunal, beastx_bot
...
from ..version import BeastX_version as ult_ver
from . import owner_and_sudos, should_allow_sudo
from ._wrappers import eod

hndlr = "\\" + HNDLR
MANAGER = mrunal.get("MANAGER")
TAKE_EDITS = mrunal.get("TAKE_EDITS")
DUAL_MODE = mrunal.get("DUAL_MODE")
black_list_chats = eval(mrunal.get("BLACKLIST_CHATS"))
            ...
            async def wrapper(ult):
                ...
                if mode in ["dualmode", "official", "sudo"]:
                    ...
                    if not allow_pm and ult.is_private:
                        ...
                        return
                if only_devs and not mrunal.get("I_DEV"):
                    return await eod(
                        ult,
                ...
                try:
                    ...
                    await asst.send_message(
                        ...
                        buttons=[
                            ...
                            Button.url(
                                "Repl",
                                "https://replit.com/@MrunalYeole/BeastX",
                            ),
                        ],
                    )
                    exit()
                    ...
                    ftext = "**BeastX Client Error:** `Forward this to` @BeastX_Support\n\n"
                    ftext += "**BeastX-Py Version:** `" + str(pyver)
                    ftext += "`\n**BeastX Version:** `" + str(ult_ver)
                    ftext += "`\n**Telethon Version:** `" + str(telever) + "`\n\n"
                    ftext += "--------START BeastX CRASH LOG--------"
                    ftext += "\n**Date:** `" + date
                    ...
                    if len(ftext) > 4096:
                        with BytesIO(ftext.encode()) as file:
                            ...
                            await asst.send_file(
                                int(mrunal["LOG_CHANNEL"]),
                                file,
                                caption="**BeastX Client Error:** `Forward this to` @BeastX_Support\n\n",
                            )
                    else:
                        await asst.send_message(
                            int(mrunal["LOG_CHANNEL"]),
