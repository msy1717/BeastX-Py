...
def add_sudo(id):  # Take int or str with numbers only , Returns Boolean
    ...
    try:
        sudos = get_sudos()
        sudos.append(id)
        mrunal.set("SUDOS", list_to_str(sudos))
        return True
    except Exception as e:
        print(f"BeastX LOG : // functions/sudos/add_sudo : {e}")
        return False
...
def del_sudo(id):  # Take int or str with numbers only , Returns Boolean
    ...
    try:
        ...
        print(f"BeastX LOG : // functions/sudos/del_sudo : {e}")
        return False
